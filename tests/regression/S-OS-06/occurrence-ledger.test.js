...
ok("pinned-occurrences-each-resolve-to-a-real-warrant", () => {
  ...
  for (const row of pinnedRows) {
    assert.ok(
      ...
      // S-OS-06 r4 lane J: a version-listing glob is COMPUTED (`computed:tag-glob`), never the evidence-tag RULE.
      ["occurrence-pin", "rule:evidence-tag", "rule:brand-history", "computed:tag-glob"].includes(row.rule),
