 ...
 *   - sanctioned --review-fallback reviewer under the NEW MC_SHAPE_DOOR=enforce → not bricked (exit 0).
...
function runClaude({ role, extra = [], env = {} }) {
  ...
  mcEnv.unsetEnv("DISPATCH_CONTRACT_ENFORCE", e); mcEnv.unsetEnv("SHAPE_DOOR", e); mcEnv.unsetEnv("DISABLE_SHAPE_DOOR", e);
  Object.assign(e, env);
...
console.log("(1) report-mode-no-regression-both-wrappers:");
{
  ...
  const r = runClaude({ role: "backend-reviewer", env: { MC_DISPATCH_CONTRACT_ENFORCE: "report" } });
  ...
  // report-only kill-switch (MC_DISPATCH_CONTRACT_ENFORCE=report|off|0) still reverts it.
  const rEnf = runClaude({ role: "backend-reviewer", env: { MC_DISPATCH_CONTRACT_ENFORCE: "enforce" } });
  ...
  ok("dispatch-claude contract =report kill-switch → exit 0 (advisory, reversible)",
    runClaude({ role: "backend-reviewer", env: { MC_DISPATCH_CONTRACT_ENFORCE: "report" } }).status === 0);
...
console.log("\n(2) dispatch-claude-sanctioned-lane-preserved (new MC_SHAPE_DOOR toggle):");
{
  // The sanctioned --review-fallback reviewer must proceed even under the NEW enforce toggle (β#1).
  const r = runClaude({ role: "backend-reviewer", extra: ["--review-fallback"], env: { MC_SHAPE_DOOR: "enforce" } });
  ok("sanctioned --review-fallback + MC_SHAPE_DOOR=enforce → exit 0 (lane not bricked)", r.status === 0, `status=${r.status} stderr=${(r.stderr || "").slice(0, 220)}`);
}
...
console.log("\n(3) two-toggle-coherence-backcompat (β#2, DoE-C2 — STRUCTURAL):");
{
  ...
  // shared contractEnforceMode helper (ADR-0013 flip: enforce-by-default + MC_DISPATCH_CONTRACT_ENFORCE=report kill).
  ...
  ok("dispatch-claude shape block no longer inlines a DISPATCH_CONTRACT_ENFORCE env check", !/process\.env\.MC_DISPATCH_CONTRACT_ENFORCE|readEnv\(\s*["']DISPATCH_CONTRACT_ENFORCE["']/.test(dcShape));
}
