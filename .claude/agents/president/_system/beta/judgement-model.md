...
| Skill-suite reconciliation (namespace collapse → one implementer + wrappers) | **0.88 (HIGH)** | New row 2026-05-26: 13 architecture consults this window, all DECIDE, 0 override (DEC-005/006 + portfolio collapse shipped + commit:land/mc:flag this session). Sustained first-pass accuracy on the dominant category. See P-034. |
...
| P-021 | Sprint-release → commit:both → mc:release fixed chain | 3 occurrences 2026-05-13: "Commit and push. Then, let's do mc:release", subsequent "push", "push main + tag", /commit:both after second sprint | MEDIUM |
| P-022 | Report-without-action triggers profanity | 7 profanity-marked events in 32h 2026-05-12..05-13; baseline ~0/week. Quote withheld (verbatim operator prompt, profane; gist: mc:update was useless, fix it instead of reporting). Direct enforcement signal | HIGH |
...
- **P-022:** Binds to A-008 below. Profanity directly precedes hotfix sprints (see Cross-source signal: mc:update UX → install-bug fix cluster). Strongest enforcement-creation signal seen in mining window.
...
| P-025 | Long autonomous skill-chain expectation under turbo | ~15 skill invocations in a single 2026-05-14 session after one "do everything" directive (clear → mode:adhoc --turbo → fix:deep → mc:flag → reasoning:run → migration → checks → learn:deep → learn:integrate → fix:deep → beta:mine → beta:integrate → release prep). Reinforces A-008 + P-019. | HIGH |
...
| ID | Pattern | Evidence | Confidence |
|---|---|---|---|
| P-034 | Overlapping skill namespaces collapse to ONE canonical implementer + thin wrappers + a 2-release deprecation window | DEC-005/006 + 2026-05-25 framing consult; 13 architecture consults this window, all DECIDE, 0 override. The `product:*`→`portfolio:*` arc; reprised 2026-05-26 (commit:both→commit:land alias, mc:flag redefinition). | HIGH |
...
12. **`/mc:release` confirmation gate inconsistent with `/sprint:release`.** Sprint releases prompt for approval (AP-NNNN); mc:release in the analysis window went through with "Commit and push. Then, let's do mc:release" — no confirmation gate fired. If a sprint hits AP-001 approval, the user may expect the same gating for the meta-framework release. Today, release-canonical.js bypasses approval. Target: clarify in `paths.decisionPolicy` or release-canonical.js whether mc:release is Class B (review rubric) or Class A (release driver, no gate).
...
| 2026-05-13 | P-021 (Sprint-release → commit:both → mc:release chain) added, DEFERRED for one more cycle | /beta:mine 2026-05-13, MEDIUM conf |
