---
description: Flag a MC framework/tooling gap from a downstream product — append a structured, canonical-consumable entry to this repo's MC.md so /warp:reconcile can verify and fix it upstream.
---
...
- **Stable filename.** Always the repo-root `MC.md`. Do not repurpose it for anything else (a product-positioning doc named `MC.md` is a collision that breaks `/warp:reconcile` discovery — keep positioning elsewhere).
...
### Step 6 — Report
Print the entry id + one-line summary, and remind: **the fix happens in canonical via `/warp:reconcile`** (run from the MC repo), which discovers this `MC.md`, verifies each gap @current, triages, and drives the fix. Note once (only if `MC.md` was just created) that canonical also ships machine-queryable stores — `paths.recurringIssuesFile` (`/issues:log`) and `paths.enforcementDebt` (`/enforcement:log`) — for teams that prefer tooling over the human-readable register.
...
- Sibling skill: [`/warp:reconcile`](reconcile.md) — the canonical-side consumer of every `MC.md` across the portfolio.
