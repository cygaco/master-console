...
# /mc:diff — Canonical ↔ Product Divergence Report
...
**Read-only and canonical-side.** This reads the product repo to compute the diff; it **never writes** to it (the MC-only / products-operate-in-their-own-session boundary — see `[[feedback_mc_only_no_cross_project]]`). To actually update a product, run `/mc:update` *inside that product's own session*.

## Usage

```
/mc:diff <slug>              # resolve the product via the portfolio registry
/mc:diff --product <path>    # diff an explicit product root
/mc:diff <slug> --json       # machine-readable report
```
...
| **framework file drift** | framework-owned files whose on-disk content differs from canonical, sub-classified: `product_stale` (unchanged since install — canonical moved on, a clean `/mc:update` would fix), `product_modified` (locally edited — an update would conflict), `missing_in_product` |
...
- **Lots of `product_stale` + `product_behind`** → the product is simply out of date; recommend `/mc:update` in its session.
...
- Never edits the product. If the diff reveals work to do, hand it off (`/portfolio:open <slug>` then `/mc:update` there).
- Builds on `/portfolio:status`, `/mc:check`, and the `/scan:mc-*` family — this is the **divergence** view that unifies "how stale" + "how diverged" + "what's missing" into one report.
