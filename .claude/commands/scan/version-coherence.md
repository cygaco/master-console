...
1. **Product-version lag** — `.claude/manifest.json#mc.version` stayed `0.10.0` after the `0.11.0` release, because `/scan:mc-version-quorum` only checks 4 sources (version.json, framework-manifest, framework-installed, install.ps1 header) — **not** `manifest.mc.version` or install.ps1's `$Script:MC_VERSION` fallback constant.
...
- **A. Product-version quorum (extended):** `version.json#version` (truth) === `.claude/manifest.json#mc.version` === `.claude/framework-manifest.json#version` === `.claude/framework-installed.json#installedVersion` === `install.ps1 $Script:MC_VERSION`.
