...
- **Installer and updater** (`install.ps1`, `scripts/warp-setup.js`, `scripts/mc/update.js`) — writes outside the target directory, overwriting operator customizations, unsafe handling of manifests.
- **Scaffold templates** (`_mc/templates/`) — anything a scaffolded product would ship insecurely (auth, session cookies, admin routes).
