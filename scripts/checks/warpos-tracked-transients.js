...
//
// Enforcer of its own contract: scripts/checks/warpos-tracked-transients.test.js
...
const ALLOWLIST_FILE = path.join(__dirname, "warpos-tracked-transients.allowlist.json");
...
function main() {
  ...
  if (allow === null) {
    const msg = `FAIL [warpos-tracked-transients] allowlist unreadable or malformed: ${ALLOWLIST_FILE} (every entry needs path + reason)`;
  ...
  // `git add`ed .warpos/ log would slip past until after it landed.
  ...
  if (violations.length === 0) {
    if (JSON_OUT) console.log(JSON.stringify({ ok: true, count: 0, allowed: allowed.length }));
      ...
      console.log(
        `OK   [warpos-tracked-transients] no transient files tracked${allowed.length ? ` (${allowed.length} allowlisted)` : ""}`,
      );
    process.exit(0);
  }
  ...
  if (JSON_OUT) console.log(JSON.stringify(out));
    ...
    console.error(
      "\nFix: for tracked files `git rm --cached <file>`; for staged files `git reset HEAD <file>`. Then ensure .gitignore covers the pattern. A file that is genuinely needed goes in scripts/checks/warpos-tracked-transients.allowlist.json WITH a reason.",
    );
  }
  process.exit(1);
}
...
module.exports = { FORBIDDEN, matchForbidden, evaluate, loadAllowlist, TRANSCRIPT_TXT_MIN_BYTES };
