...
"use strict";
/**
 * scripts/checks/mc-ship-coverage.js — THE systemic enforcer for the
 ...
 *
 * Root cause it closes: MC has TWO manifests.
 ...
 *
 * This check diffs them: every owner=framework path in _mc/MANIFEST.json MUST
 ...
 *
 * Also asserts seeded_from integrity: every `seeded_from` value in _mc/MANIFEST.json
 ...
 *
 * Wire into scripts/mc/release-gates.js so "forgot to ship X" fails the
...
// _mc/templates/ end-state lands, SP-20260522-001) must be added here as an EXACT
...
const KNOWN_NOT_SHIPPED = [
  // _mc/ is the framework source-of-truth zone. Most of it SHIPS from canonical
  // (e.g. _mc/templates/* — the master seeds — must fall through to the shipped
  ...
  // (the old blanket `_mc/` entry masked exactly that gap).
  { prefix: "_mc/MANIFEST.json", reason: "build-output ownership manifest, not shipped from canonical" },
  { prefix: "_mc/settings/", reason: "layered-settings compiler source, not shipped from canonical" },
  ...
  // _mc/templates/ still falls through to the shipped set.
  { prefix: "_mc/BASELINE/", reason: "build-output seed snapshot for the deferred validate.js:30 seed-drift diff; not shipped from canonical (per-install generator pending)" },
  ...
  // _mc/templates/ still falls through to the shipped set.
  { prefix: "_mc/EXAMPLES/", reason: "W4 labeled reference examples — UNTRACKED + gitignored in S-OS-04 (private product tree); rule retained so a re-tracked copy can never ship; the synthetic example lives at _mc/BASELINE/; consumers generate their own canon" },
  // MC-as-product canon — root-leak pending 0.10.0 scrub; product content,
  // never shipped (consumers generate their own via the canon engine).
  { prefix: "_requirements/00-canonical/", reason: "MC product canon (root-leak pending scrub); consumers generate their own" },
  ...
  // scripts/hooks/, scripts/mc/, scripts/checks/ etc. are separately governed
...
// boundary between MC-authored product-facing guides (MUST ship) and operator
...
function fail(msg) {
  console.error(`mc-ship-coverage: ${msg}`);
  process.exit(2);
}
...
function main() {
  ...
  // _guides/** MUST ship (MC-authored product-facing guides); _planning/** +
  ...
  if (JSON_OUT) {
    ...
    if (result.ok) {
      console.log(
        `OK   [mc-ship-coverage] every framework-owned path ships or is allowlisted (${frameworkOwned} paths scanned, 0 hard_gaps, 0 info_gaps, 0 boundary_violations); seeded_from: ${danglingTotal} dangling (0.16.0: prefix-drift reconciled in the generator; KNOWN_DANGLING_SET empty; zero-tolerance).`,
      );
    } else {
      if (hardGaps.length) {
        console.error(
          `FAIL [mc-ship-coverage] ${hardGaps.length} essential-root path(s) ship to NOBODY (under framework/|schemas/|patterns/|commands|agents, not shipped, not allowlisted):`,
        );
      ...
      if (infoGaps.length) {
        ...
        console.error(
          `FAIL [mc-ship-coverage] ${infoGaps.length} owner=framework path(s) are not shipped and not in KNOWN_NOT_SHIPPED (unallowlisted — every exclusion must be a conscious reviewed decision):`,
        );
      ...
      if (boundaryViolations.length) {
        console.error(
          `FAIL [mc-ship-coverage] ${boundaryViolations.length} ship-boundary violation(s) (SP-20260531-002):`,
        );
      ...
      if (danglingUnallowlisted.length) {
        console.error(
          `FAIL [mc-ship-coverage] ${danglingUnallowlisted.length} seeded_from pointer(s) are dangling and NOT in KNOWN_DANGLING_SET (new regression — exact-match required):`,
        );
    ...
    if (danglingTotal > 0 && danglingUnallowlisted.length === 0) {
      console.log(
        `INFO [mc-ship-coverage] seeded_from: ${danglingTotal} dangling pointer(s), all ${danglingAllowlisted.length} in KNOWN_DANGLING_SET (each an EXACT-string allowlist entry tied to a tracked sprint).`,
      );
    }
  }
