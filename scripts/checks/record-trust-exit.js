 ...
 *                a FAIL, never a zero), AND the derived rule holds: the partition declares exactly 5
 *                generated views, read at their RESOLVED paths (declared, or the codemod's rename of
 *                it once --apply moved the view with its Class-1 directory — revisited post-apply in
 *                T3); every file carrying `derived` rows in the committed occurrence ledger this run
 *                just wrote is one of them; and every view is EITHER derived-carrying OR regenerated
 *                clean — present on disk with ZERO rows (any disposition) in the full occurrence
 *                ledger this same run wrote (revisited at T4: a view regenerated from the mc SOURCE
 *                legitimately carries no legacy slug; a view whose hits were pinned/rewritten instead
 *                of derived, a missing view, or an unreadable full ledger is still a FAIL); the
...
// The codemod's FULL occurrence ledger (every disposition, regenerated by each --dry-run, never committed) — the
// derived rule reads "a generated view carries no legacy slug" from it (rename-mc.js#FULL_LEDGER_REL).
...
async function checkDryRun({ root }) {
  ...
  // Derived rule (revisited post-apply in T3, as the header required; revisited again at T4 once the paths views
  // were regenerated from the mc SOURCE): the partition declares exactly 5 generated views, each read at its
  // RESOLVED path — the declared path, or the codemod's rename of it once --apply has moved the view with its
  // Class-1 directory. The loader resolves a view entry through renamePath only (like a pin), so this never
  // admits a path beyond the 5 declared entries. Every view is then EITHER derived-carrying OR regenerated clean
  // (derivedRuleProblems).
  let views;
  try {
  ...
  // The FULL occurrence ledger this same run wrote (every disposition) is the codemod's own scan: "this view carries
  // no legacy slug" is read from it, so this enforcer never grows a second needle literal. Stale/unreadable = FAIL.
  let fullRows = null;
  try {
    const fullAbs = absOf(root, FULL_LEDGER_REL);
    const st = fs.statSync(fullAbs);
    if (st.mtimeMs + MTIME_SLACK_MS < startedAt) problems.push(`${FULL_LEDGER_REL} was not rewritten by this dry-run (stale full ledger)`);
    const full = JSON.parse(fs.readFileSync(fullAbs, "utf8").replace(/^﻿/, ""));
    if (Array.isArray(full.rows)) fullRows = full.rows;
    else problems.push(`full ledger ${FULL_LEDGER_REL} has no rows array (fail-closed)`);
  } catch (e) {
    problems.push(`full ledger ${FULL_LEDGER_REL} unreadable: ${e.message}`);
  }
  if (ledger) {
    if (!Array.isArray(ledger.rows)) {
      ...
      const rule = derivedRuleProblems({ views, derivedFiles, fullRows, viewExists: (v) => fs.existsSync(absOf(root, v)) });
...
/**
 * The derived rule over resolved view paths (pure — unit-tested in record-trust-exit.test.js).
 *   - a `derived` row on a file that is NOT a generated view                      -> FAIL (the false-green class)
 *   - a view carrying derived rows                                                -> ok
 *   - a view with NO derived rows: ok ONLY if it exists on disk AND has zero rows of ANY disposition in the full
 *     ledger (regenerated clean from the mc SOURCE — T4). Hits disposed pinned/rewritten instead of derived, a
 *     missing view, or an unavailable full ledger (nothing can be verified clean) -> FAIL.
 * -> { problems: string[], cleanViews: string[] }
 */
function derivedRuleProblems({ views, derivedFiles, fullRows, viewExists }) {
  const problems = [];
  const cleanViews = [];
  ...
  const nonDerived = views.filter((v) => !derivedFiles.includes(v));
  const missing = nonDerived.filter((v) => !viewExists(v));
  if (missing.length) problems.push(`generated view(s) missing on disk: ${missing.join(", ")}`);
  const present = nonDerived.filter((v) => viewExists(v));
  if (!Array.isArray(fullRows)) {
    if (present.length) problems.push(`generated view(s) with no derived occurrence cannot be verified clean (full occurrence ledger unavailable): ${present.join(", ")}`);
    return { problems, cleanViews };
  }
  const dirty = [];
  for (const v of present) {
    const hits = fullRows.filter((row) => row && toPosix(row.file) === v);
    if (hits.length === 0) {
      cleanViews.push(v);
      continue;
    }
    const dispositions = [...new Set(hits.map((row) => row.disposition))].sort().join("/");
    dirty.push(`${v} (${hits.length} occurrence(s) disposed ${dispositions})`);
  }
  if (dirty.length) problems.push(`generated view(s) with no derived occurrence: ${dirty.join(", ")}`);
  return { problems, cleanViews };
}
