...
- [x] `--where android|ios|web|desktop-pc|desktop-mac` accepted; recorded in brief + canon PRODUCT_MODEL; influences roadmap; passed to scaffold (web/PWA baseline for all v1 + native-packaging epic + `/warp:flag`). — `setup.js` threads it; `E-NATIVE-PACKAGING-001` + `MC.md` WG-1.
